...
//! Optimizer rule that moves expensive projection expressions above a limit/sort pair.
//! Cube note: This seems useful in combination with Cubestore optimizations like materialize_topk.

use crate::error::Result;
use crate::execution::context::ExecutionProps;
use crate::logical_plan::{
    replace_col, Column, DFField, DFSchema, Expr, ExpressionVisitor, LogicalPlan,
    Recursion,
};
...
/// Optimizer that moves Projection calculations above Limit/Sort.
        ...
        LogicalPlan::Projection {
            ...
            for (i, ex) in expr.iter().enumerate() {
                ...
                if let Expr::Column(col) = ex {
                    pal_debug!(
                        "Expr {} added to already_retained_cols: {:?}",
                        i,
                        col
                    );
            ...
            'outer: for col in needed_columns {
                ...
                if !used_field_names.contains(input_field.name()) {
                    ...
                    'this_loop: loop {
                        let proposed =
                            format!("p_a_l_generated_{}", generated_col_number);
...
mod tests {
    ...
    use super::*;
    use crate::logical_plan::{col, lit, when, LogicalPlanBuilder};
    use crate::test::*;
    ...
    fn limit_sorted_plan_with_expr_referencing_column() -> Result<()> {
        ...
        let optimized_expected =
            "Projection: #a1, #d1, (- #p_a_l_generated_0) AS (- d1)\
        \n  Limit: 50\
        \n    Sort: #a1 ASC NULLS FIRST\
    ...
    fn optimize(plan: &LogicalPlan) -> Result<LogicalPlan> {
        let rule = ProjectionAboveLimit::new();
